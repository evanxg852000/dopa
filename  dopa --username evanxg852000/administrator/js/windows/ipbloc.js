MyDesktop.IPbloc = Ext.extend(Ext.app.Module, {
	...
	// variable edition du gride_ip
		...
		var etat_edit = new Ext.form.ComboBox({store:['Y','N'],editable: false,triggerAction: 'all'});
							...
							handler: function(){
												...
												if (sm.hasSelection()){
													Ext.Msg.show({
														title: 'Dopa', 
														buttons: Ext.MessageBox.YESNOCANCEL,
														msg: 'Etes vous certain de vouloir debloquer '+sel.data.ipCon+'?',
														fn: function(btn){
															if (btn == 'yes'){
																...
																conn.request({
																	...
																	failure: function(resp,opt) { 
																		Ext.Msg.alert('Error','Impossible de debloquer l\' IP!'); 
																	}
																});
															}
														}
...
//creation de la fenetre 				
        ...
        if(!win){
			...
			var mask_ip = new Ext.LoadMask(Ext.get('grid-win'), {store:store_ip,msg:'Chargement...'});
			store_ip.load();
        }
        win.show();
    }
